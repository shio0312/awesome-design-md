...
> **North Star**: graphite ink on warm paper
> **Theme**: light
...
- **Sidebar Mist**: `#f9f9f9` — Sidebar and secondary surface backgrounds — page chrome recedes behind the conversation [neutral]
- **Pure White**: `#ffffff` — Main canvas and elevated surface — primary reading area, elevated panels, inverted text backgrounds [neutral]
- **Graphite Ink**: `#0d0d0d` — Primary headings, body text, and icon fills on light surfaces. Do not promote it to the primary CTA color [neutral]
- **Mid Ash**: `#5d5d5d` — Secondary text and icons — supporting labels, metadata [neutral]
- **Hollow**: `#8f8f8f` — Tertiary text, disabled states, muted helper copy [neutral]
- **Hairline**: `#0000001a` — 1px borders and dividers — rgba black at ~10% opacity, the sole structural separator [neutral]
- **Hover Veil**: `#0000000d` — Supporting palette color for small decorative accents when the core palette needs contrast. Do not promote it to the primary CTA color [neutral]
- **Ink Press**: `#000000` — Pressed/inverted surface fill, tooltips, scrim — true black for max contrast moments [neutral]
- **Deep Charcoal**: `#00000080` — Modal scrim overlay — 50% black behind dialogs and drawers [neutral]
- **Edge Gray**: `#e6e6e6` — Stronger divider or inactive surface — used sparingly as a higher-contrast border alternative [neutral]
...
- **Element Gap**: 6px
- **Section Gap**: 24px
- **Border Radius**: {'nav': '10px', 'cards': '10px', 'links': '16px', 'badges': '0px', 'buttons': '10px'}
...
- {'why': 'Same achromatic-by-default philosophy with a single subtle background tint for the sidebar; relies on hairline borders and weight rather than color to communicate hierarchy', 'business': 'Linear'}
- {'why': 'Near-identical system font usage, 10-12px radius on controls, grayscale-only interface where the document content provides all visual interest', 'business': 'Notion'}
- {'why': 'Minimal utility UI with hairline 1px borders instead of shadows, system fonts, and zero chromatic accents in the chrome', 'business': 'Vercel'}
- {'why': 'Quiet grayscale sidebar + white main surface, restrained typographic scale, and structural separation through dividers rather than elevation', 'business': 'Things 3 (Cultured Code)'}
