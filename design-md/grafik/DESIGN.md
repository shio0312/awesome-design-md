...
- **Bone**: `#f0eeeb` — Page background and primary canvas — a warm off-white that reads as paper rather than screen, the defining surface of the entire system [neutral]
- **Ink**: `#000000` — Primary text, body copy, navigation, project metadata, 1px grid lines, image borders, and dark image treatments — the only non-canvas color [neutral]
- **Paper**: `#ffffff` — Card surfaces, image backgrounds, and reverse-text blocks — white inserts on the warm canvas to isolate portfolio pieces [neutral]
...
- **Element Gap**: 12px
- **Section Gap**: 80px
- **Border Radius**: {'all': '0px — no rounded corners anywhere in the system', 'cards': '0px', 'images': '0px', 'buttons': '0px'}
...
- {'why': 'Editorial portfolio aesthetic with a single weight per page, hairline grid structure, and monochromatic restraint — same gallery-on-paper approach to showcasing work', 'business': 'Pentagram'}
- {'why': 'Minimal text-only navigation, full-bleed project imagery, and a typographic system built on a single sans-serif at regular weight with negative tracking', 'business': 'Manual (manual.to)'}
- {'why': 'Swiss-design sensibility with sharp 0px corners, monochromatic interface, and oversized black-and-white photography in a rigid grid', 'business': 'DIA Studio'}
- {'why': 'Editorial typography-first portfolio with hairline dividers, no decorative shadows, and a canvas that reads as paper rather than screen', 'business': 'Studio Dumbar'}
- {'why': 'Full-bleed project showcases with device-framed screenshots, monochromatic chrome, and generous vertical breathing room between sections', 'business': 'Ramotion (agency site)'}
