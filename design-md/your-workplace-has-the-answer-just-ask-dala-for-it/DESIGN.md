# Your workplace has the answer. Just ask Dala for it. — Design System

> **North Star**: constellation floating on black velvet
> **Theme**: dark
...
- **Void**: `#000000` — Page canvas, section backgrounds, negative space — pure black is the dominant surface, not dark gray, creating the void that lets chromatic accents float [neutral]
- **Bone White**: `#ffffff` — Headlines, body text, icon fills, nav active state — the only typographic color, carrying maximum hierarchy on black [neutral]
- **Ash Gray**: `#9a9a9a` — Muted nav text, ghost link color, secondary labels — recedes behind primary text without going invisible [neutral]
- **Silver Mist**: `#bdbdbd` — Tertiary body text, caption-level information — the quietest readable gray, for supporting context [neutral]
- **Electric Iris**: `#8052ff` — Primary action buttons, logo mark, brand accents — the single saturated violet that signals interactivity and brand identity against the black void [brand]
- **Saffron Spark**: `#ffb829` — Highlight emphasis text, accent links, attention punctuation — warm yellow against violet creates the brand's chromatic tension [accent]
- **Deep Verdant**: `#15846e` — Secondary surface tint, logo gradient stop — appears as the deeper end of the brand gradient and in subtle accent washes [accent]
