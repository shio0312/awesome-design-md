...
> **North Star**: Phosphor terminal in a darkened server room — the vivid green is the only light source.
> **Theme**: dark
> **Source**: https://modal.com
> **Refero Style**: https://styles.refero.design/style/68c15685-5db9-4869-b71d-27240568c9d8
> **Synced**: 2026-09-01
...
- **Void Black**: `#000000` — Page canvas, deepest background layer [neutral]
- **Ground Iron**: `#181818` — Primary button fill, card surfaces on dark sections, base UI surface [neutral]
- **Carbon Veil**: `#212525` — Elevated surfaces, nav background, subtly lighter than ground [neutral]
- **Circuit Border**: `#485346` — Outlined/ghost action border, primary interactive hairline — muted green-tinted gray [neutral]
- **Phosphor Blue-Black**: `#1f2a33` — Secondary hairline borders, separator strokes between dark bands [neutral]
- **Charcoal Rust**: `#231c1c` — Subtle warm-tinted dark accent for code-window inner panels [neutral]
- **Lime Pulse**: `#7fee64` — Primary brand accent — logo mark, hero CTA pill, tag fills, active states, illustration base color [brand]
- **Phosphor White**: `#ddffdc` — Primary text, heading color, icon strokes, filled button text — the dominant readable surface [accent]
- **Mint Frost**: `#def0dd` — Card surface tint on light sections, secondary text on dark, soft highlight wash [accent]
- **Sage 60**: `#8cab87` — Body copy default — readable muted green-gray for paragraphs on dark canvas [accent]
- **Sage 40**: `#677d64` — Muted helper text, secondary descriptions, low-priority body content [accent]
- **Moss 70**: `#9cbf93` — Eyebrow labels, category tags above section headings [accent]
- **Moss 80**: `#aed2a4` — Lead paragraph text, hero subhead — the brightest muted green for lead copy [accent]
- **Fern Link**: `#859984` — Inline link text, tertiary navigation links on dark [accent]
- **Deep Fern**: `#697368` — Small uppercase labels, micro-copy, footer text [accent]
- **Pine 15**: `#3e4a3c` — Secondary outlined action border, low-emphasis interactive hairline [neutral]
...
- **Element Gap**: 20px
- **Section Gap**: 80px
- **Border Radius**: {'cards': '8px', 'icons': '0px', 'pills': '9999px', 'inputs': '8px', 'buttons': '12px'}
...
- {'why': 'Same dark canvas with a single vivid green accent, developer-first code-window imagery, and a rationed-use approach to chromatic color', 'business': 'Replicate'}
- {'why': 'Dark-mode AI infrastructure with a lime/emerald accent, similar generous spacing, and a custom geometric display face for headings', 'business': 'Together AI'}
- {'why': 'Dark-by-default dev tool with a single bright accent color, flat surfaces, and hairline borders instead of shadows for depth', 'business': 'Railway'}
- {'why': 'Dark mode marketing site with generous whitespace, centered hero stacks, and 2-column feature sections with code/visual cards', 'business': 'Vercel'}
- {'why': 'Developer-focused AI infrastructure with a green-on-dark phosphor aesthetic and 3D illustration accents', 'business': 'Anyscale'}
