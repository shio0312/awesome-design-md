...
> **North Star**: midnight workspace with coral embers — a quiet aubergine app surface where one warm orange spark signals every action
> **Theme**: dark
...
- **Aubergine Canvas**: `#181824` — Page background, primary canvas — warm-tinted dark replaces pure black so the UI feels layered rather than flat [neutral]
- **Elevated Plum**: `#26253b` — Card surfaces, input fields, nav containers — one step lighter than canvas to create depth without a visible border [neutral]
- **Pure White**: `#ffffff` — Primary headings, button text, icon strokes — maximum contrast on the dark canvas [neutral]
- **Fog Text**: `#696f81` — Body text, secondary headings, muted borders — sits at 7:1 contrast for comfortable reading on the aubergine canvas [neutral]
- **Ash Text**: `#8e8da0` — Supporting neutral for secondary UI, dividers, and muted labels. Do not promote it to the primary CTA color [neutral]
- **Mist Highlight**: `#a3a3a7` — Subtle borders, dividers, inactive nav items — the faintest neutral that still reads against canvas [neutral]
- **Whisper Border**: `#f7f7ff` — Hairline highlights on dark surfaces, inset strokes — a near-white that reads as a light edge in dark mode [neutral]
- **Coral Ember**: `#ff4a36` — Orange supporting accent for decorative details and low-frequency emphasis. Do not promote it to the primary CTA color [brand]
- **Iris Accent**: `#535676` — Secondary headings, decorative violet text — a muted blue-violet that harmonizes with the aubergine canvas without competing with coral [accent]
...
- **Element Gap**: 10-16px
- **Section Gap**: 80px
- **Border Radius**: {'tags': '100px', 'cards': '20px', 'images': '20px', 'inputs': '8px', 'buttons': '100px'}
...
- {'why': 'Same dark-mode discipline with a single accent color, oversized display headlines, and pill-shaped primary buttons. Both treat the interface as a canvas for typography rather than a dense data grid.', 'business': 'Linear'}
- {'why': 'Productivity tool with a dark canvas and minimal chromatic accents. Shares the generous spacing rhythm and the strategy of letting product screenshots carry the visual weight.', 'business': 'Notion'}
- {'why': 'Dark default interface with warm-tinted surfaces (not pure black), editorial-scale typography, and a confident use of one accent color. Both feel like products designed for creative professionals who work at night.', 'business': 'Arc Browser'}
- {'why': 'Dark productivity interface with similar rounded product frames, pill buttons, and a restrained color palette. Both use the product screenshot as the primary hero visual rather than abstract graphics.', 'business': 'Cron (calendar app)'}
- {'why': 'Dark-mode productivity tool with clean centered layouts, large display headlines, and a minimal accent palette. Both prioritize typographic confidence over decorative elements.', 'business': 'Raycast'}
