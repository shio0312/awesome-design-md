...
> **North Star**: dusk-lit workspace with frosted glass panels
> **Theme**: dark
> **Source**: https://www.dimension.dev
> **Refero Style**: https://styles.refero.design/style/fbcf9cbb-7c6b-449d-862a-bce521a8ab1d
> **Synced**: 2026-09-01
...
- **Void Canvas**: `#0a0a0a` — Primary page background, base surface for the dark UI shell [neutral]
- **Graphite**: `#161616` — Elevated surface for floating panels, pill nav bar, modal bodies — one step above canvas [neutral]
- **Frosted Glass**: `#d4d4d4` — Translucent panel fill at 10% opacity over dark surfaces — creates the frosted overlay look on the floating nav and feature cards [neutral]
- **Ink Black**: `#000000` — Icon strokes, SVG fill, deep contrast elements on light surfaces [neutral]
- **Snow White**: `#ffffff` — Primary CTA fill, headline text on dark surfaces, card backgrounds — the inverted counterweight to the dark canvas [neutral]
- **Bone**: `#ededed` — Primary readable text on dark surfaces — slightly warmer than pure white, reduces glare [neutral]
- **Ash**: `#c2c2c2` — Secondary body text, helper copy, de-emphasized labels [neutral]
- **Slate**: `#686868` — Muted text, link text, tertiary metadata, timestamp-style labels [neutral]
- **Smoke**: `#b2b2b2` — Disabled or idle button text, integration label text in ghost controls [neutral]
- **Hairline**: `#e5e5e5` — 1px borders on dark-surface controls — appears as a light edge against the dark canvas, creating definition without weight [neutral]
- **Dusk Violet**: `#6b62f2` — The only chromatic accent — used at 0.565 alpha in a horizontal gradient wash to add a single pulse of color to the dark UI; Radial glow accent — concentrated violet center fading to white, used as spotlight or product reveal moment [accent]
...
- **Element Gap**: 8-16px
- **Section Gap**: 64-80px
- **Border Radius**: {'ui': '10px', 'cards': '24px', 'icons': '4px', 'panels': '42px', 'buttons': '9999px', 'largeCards': '40px'}
...
- {'why': 'Same dark-canvas + monochromatic discipline with a single gradient hero, pill-shaped controls, and hairline borders on dark surfaces. Both favor weight-500 restraint over bold headlines.', 'business': 'Linear'}
- {'why': 'Identical Geist/DM Sans pairing, 9999px pill buttons, 1px hairline borders on #e5e5e5, and the floating glass nav bar concept. Both use warm-to-cool gradient heroes as signature moments.', 'business': 'Vercel'}
- {'why': 'Dark surface stack (#0a0a0a → #161616), pill nav controls, frosted glass panels with backdrop blur, and the same anti-bold weight-500 headline approach. Both treat 2% colorfulness as a design principle.', 'business': 'Raycast'}
- {'why': 'Shared language of frosted glass panels over dark canvases, pill-shaped floating controls, and generous editorial spacing. Both use gradient washes as accent rather than as primary fills.', 'business': 'Arc Browser'}
